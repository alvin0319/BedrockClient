...
namespace alvin0319\BedrockClient\encryption;
...
class EncryptionContext{
	...
	/**
	 * @throws DecryptionExceptionBase
	 */
	public function decrypt(string $encrypted) : string{
		if(strlen($encrypted) < 9){
			throw new DecryptionExceptionBase("Payload is too short");
		}
		...
		if(($expected = $this->calculateChecksum($packetCounter, $payload)) !== ($actual = substr($decrypted, -8))){
			throw new DecryptionExceptionBase("Encrypted packet $packetCounter has invalid checksum (expected " . bin2hex($expected) . ", got " . bin2hex($actual) . ")");
		}
